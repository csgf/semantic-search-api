...
public class GetSingleResource {
    ...
    public ResourceVirtuoso getSingleResource(@QueryParam("id") String idResource) throws IOException {
        //ArrayList listResource = new ArrayList();


        System.out.println("Start to Get List OF Resource Virtuoso with id =" + idResource);
        ...
        try {
            ...
            for (int i = 0; i < identifiers.size(); i++) {
                if (identifiers.get(i).toString().contains("doi")) {
                    if (!(identifiers.get(i).toString().contains("info"))) {
                        doi = identifiers.get(i).toString().substring(5);
                    } else {
                        doi = identifiers.get(i).toString().substring(9);
                    }
